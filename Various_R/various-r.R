...
ggplot(
  ...
  ggdark::dark_mode()+
  labs(title = "shape and scale values (Riffomonas tutorial)")


# 6JT5WjNXK5elyGhk2AsE
# 6JT5WjNXK5elyGhk2AsE


moods = data.frame(
  dates = seq(ymd("2021-12-1"), ymd("2022-12-1"), by = "day"),
  mood = rep('-',366)
)
write.csv(moods, file = "moods.csv", append = T)
...
spiders %>% 
  select(family, distribution) %>%
  filter(distribution =="Canada")
